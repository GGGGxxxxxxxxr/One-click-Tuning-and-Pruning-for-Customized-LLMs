        ...
        **kwargs,
        ...
        if self.training != True:
            if pruning_out_mask != None:
                attn_output = self.dense(attn_output) * pruning_out_mask
            else:
...
class PhiMLP(nn.Module):
    ...
    def forward(self, 
                hidden_states: torch.Tensor,
                mask_up: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        if self.training != True:
            if mask_up != None:
                ...
                hidden_states = self.fc1(hidden_states) * mask_up
                hidden_states = self.activation_fn(hidden_states)
                hidden_states = self.fc2(hidden_states)
            else:
                hidden_states = self.fc1(hidden_states)
...
class PhiDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        feed_forward_hidden_states = self.resid_dropout(self.mlp(hidden_states, m_umlp))
...
class PhiModel(PhiPreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # layer_idx for layer_wise mask extraction
        layer_idx = 0
...
# updated as ATP's compatible pruning-aware forward()
