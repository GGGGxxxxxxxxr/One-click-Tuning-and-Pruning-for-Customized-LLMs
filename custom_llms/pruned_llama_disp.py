...
from util_llm import transform_output_layer_DISP
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        layer_idx: Optional[int] = None,                                          # layer_idx for extracting the corresponding mask for cur_layer
...
def replace_linear(layer, in_mask=None, out_mask=None):
    """
    Replace a standard nn.Linear layer with a pruned version **without modifying weights**.

    Args:
        layer (nn.Linear): The original linear layer.
        in_mask (torch.Tensor, optional): A binary mask for input pruning.
        out_mask (torch.Tensor, optional): A binary mask for output pruning.

    Returns:
        nn.Linear: A new pruned Linear layer with the expected dimensions (but no weight modifications).
    """
    assert isinstance(layer, nn.Linear), "Expected a nn.Linear layer."
    ...
    # Create a new linear layer with the correct shape (weights will be loaded from checkpoint)
    pruned_linear = nn.Linear(pruned_in_dim, pruned_out_dim, bias=False).to("cpu")
...
class PrunedLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config, pruning_masks):
        """
        Initialize PrunedLlamaForCausalLM by first calling the original LlamaForCausalLM
        constructor, then applying pruning modifications.
        """
    ...
    def forward(
        ...
        for i, size in enumerate(lw_structure):
            ...
            sliced_input_tensor = self.pruning_masks[:, start:end]
...
def model_replace(prunedllama, model_name):
    ...
    for layer_idx, decoder_layer in enumerate(prunedllama.model.layers):
        ...
        # **Replace QKV Projections (no weight changes)**
        cur_layer.self_attn.q_proj = replace_linear(cur_layer.self_attn.q_proj, in_mask=m_s1)
        cur_layer.self_attn.k_proj = replace_linear(cur_layer.self_attn.k_proj, in_mask=m_s1)
        cur_layer.self_attn.v_proj = replace_linear(cur_layer.self_attn.v_proj, in_mask=m_s1)

        # **Replace O Projection**
        cur_layer.self_attn.o_proj = replace_linear(cur_layer.self_attn.o_proj, out_mask=m_s2)

        # **Replace MLP Layers**
        cur_layer.mlp.gate_proj    = replace_linear(cur_layer.mlp.gate_proj, in_mask=m_s3, out_mask=m_s4)
        cur_layer.mlp.up_proj      = replace_linear(cur_layer.mlp.up_proj, in_mask=m_s3, out_mask=m_s4)
