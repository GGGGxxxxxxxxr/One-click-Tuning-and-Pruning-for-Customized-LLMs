...
parser.add_argument('--control-epochs', default=15, type=int,
...
def main():
    ...
    # ** llama tokenizer does not have a specific PAD token, so a special pad token is appended here for string length alignment
        ...
        model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf", attn_implementation="sdpa", use_auth_token=api_token).cuda()
    ...
    hyper_net = LLM_HyperStructure(p_structure = p_structures, T = 0.4, base = 3, args = args)
    hyper_net.cuda()
    ...
    hyper_net_ddp   = DDP(hyper_net, device_ids=[rank])
    ...
    llm_ddp         = DDP(model, device_ids=[rank])
