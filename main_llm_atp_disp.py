...
from hypernet_llm import LLM_HyperStructure, LLM_HyperStructure_v2, LLM_HyperStructure_v3
...
def save_checkpoint(
    ...
    cur_mask_vec,
    model=None, 
    filename="/orange/sgao1/atp_llm_dir/llm_atpdisp_lora_qa_10000cali.pth.tar"
):
    """
...
def main():
    ...
    hyper_net = LLM_HyperStructure_v3(p_structure = p_structures, T = 0.2, base = 3, args = args).to(device)#.to(dtype=torch.bfloat16).to(device)
    ...
    if args.use_8bit_training == True:
        ...
        optimizer_hyper = torch.optim.AdamW(hyper_params,  lr  = 5e-5)
