...
parser.add_argument('--start-epoch-regularization', default=999999, type=int,
...
def save_checkpoint(
    epoch, 
    model=None, 
    filename="/orange/sgao1/base_lora.pth.tar"
):
    """
    ...
    if model is not None:
        state['model_state_dict'] = (
            model.module.model.state_dict() if hasattr(model, "module") else model.state_dict()
        )
...
def main():
    ...
    if args.model == 'qwen-0.5b':
        ...
        model     = PhiForCausalLM.from_pretrained("microsoft/phi-2", torch_dtype=torch.bfloat16).to(device)
